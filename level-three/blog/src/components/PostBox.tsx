...
const PostBox: FC<PostProps> = ({
    ...
    return (
        <div className='relative flex items-center w-full max-w-full p-4 overflow-hidden bg-white border rounded-lg hover:bg-neutral-50 md:rounded-xl hover:border-slate-300 md:px-6 md:py-4 group'>
            {posts?.map((post) => (
                <Link key={post.id} href={`/p/${post.id}`} passHref className='flex flex-col items-start w-full'>
                    <div className='flex flex-col items-start w-full gap-y-4'>
                        ...
                        <div className='flex flex-col items-start'>
                            ...
                            <div className='relative w-full max-w-xl truncate max-h-40'>
